...
\begin{frame}{Introduction \& Problématique}
    \begin{columns}
        \begin{column}{0.55\textwidth}
            \textbf{Le Contexte : Solveurs Itératifs}
            \begin{itemize}
    ...
    \begin{columns}
        \begin{column}{0.45\textwidth}
            ...
            \begin{itemize}
                ...
                \item Optimum localisé à $\varepsilon \approx 1.47 \times 10^{-3}$ (fill-in $= 4.04$, 18 itérations).
            \end{itemize}
        \end{column}
        ...
        \begin{column}{0.4\textwidth}
            \begin{block}{Complexité arithmétique additionnelle}
                \begin{itemize}
                    ...
                    \end{itemize}
                    \item \textbf{Coût global} : $O(\text{nnz}(L) + \text{nnz}(U))$, soit seulement 1 à 2 flops par élément stocké.
                    \item \textbf{Totalement négligeable} devant le coût d'ILUT ($O(\frac{\text{nnz}(L+U)^2}{n})$) et de GMRES ($O(m \cdot \text{nnz}(L+U) + m^2 n)$) !
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Résultats : Robustesse spectrale et Remplissage}
    \begin{columns}
        \begin{column}{0.45\textwidth}
            \textbf{Observations :}
            \begin{itemize}
                \item \textbf{Gauche (Itérations)} : GEP Mixte (courbe bleue) suit de très près le FP64 de référence (écart $\le 1$ itération dû au bruit d'arrondi sur les éléments compressés). Il n'y a \textbf{aucune divergence}, contrairement au FP16 uniforme.
...
\begin{frame}{Résultats : Profil de Performance Dolan-Moré -- Mémoire}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item \textbf{Profil Global (avec ILU(0))} : GEP Mixte (bleu) domine largement tous les solveurs robustes en efficacité mémoire.
            \end{itemize}
        \end{column}
...
\begin{frame}{Résultats : Profil de Performance Dolan-Moré -- Itérations}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item GEP Mixte (bleu) offre exactement la même robustesse face aux itérations que les solveurs double précision (FP64).
            \end{itemize}
        \end{column}
