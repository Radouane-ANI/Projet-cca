...
\title{\textbf{Incomplete Factorizations of Sparse Matrices in Mixed Precision}\\
\large\textit{CCA Course Report -- Under the Supervision of Théo Mary (LIP6)}\\
\large\textit{Academic Year 2025-2026}}
\author{\textbf{Radouane Aouini et Sami Hbabou}}
\date{\today}
...
\subsection{Theoretical Foundations of Local Dropping Errors (\texttt{preuve.pdf})}%
\label{sec:dropping_bound}

The theoretical foundation of threshold dropping lies in local error analysis. Let $\hat{L}$, $\hat{U}$ be the incomplete factors computed by ILUT, such that $\hat{L}\hat{U} = A + E$, where $E$ is the error matrix. The following theorem, proved in \texttt{preuve.pdf}, establishes the key bound.

\textbf{Theorem.} \textit{For any element $(i,j)$ dropped during ILUT elimination, the local error satisfies:}
\begin{equation}
    |E_{ij}| \le \varepsilon \|A_{\cdot,j}\|_2
\end{equation}

\textbf{Proof.} Let $v_{ij}$ denote the updated value of element $(i,j)$ just before the drop decision, i.e.:
\begin{equation}
    v_{ij} = a_{ij} - \sum_{k=1}^{\min(i,j)-1} \hat{l}_{ik}\hat{u}_{kj}
\end{equation}

\textit{Case 1 -- Element dropped from $U$ ($i \le j$):} Here $\min(i,j) = i$ and $\hat{l}_{ii} = 1$. The algorithm sets $\hat{u}_{ij} = 0$ whenever $|v_{ij}| \le \varepsilon\|A_{\cdot,j}\|_2$. Expanding the error:
\begin{align}
    E_{ij} &= (\hat{L}\hat{U})_{ij} - a_{ij} = \sum_{k=1}^{i-1}\hat{l}_{ik}\hat{u}_{kj} + \underbrace{\hat{l}_{ii}}_{=1}\underbrace{\hat{u}_{ij}}_{=0} - a_{ij} \\
    &= -\left(a_{ij} - \sum_{k=1}^{i-1}\hat{l}_{ik}\hat{u}_{kj}\right) = -v_{ij}
\end{align}
The drop condition gives directly:
\begin{equation}
    |E_{ij}| = |v_{ij}| \le \varepsilon\|A_{\cdot,j}\|_2
\end{equation}

\textit{Case 2 -- Element dropped from $L$ ($i > j$):} Here $\min(i,j) = j$. The multiplier $m_{ij} = v_{ij}/\hat{u}_{jj}$ is dropped when $|m_{ij}| \le \frac{\varepsilon\|A_{\cdot,j}\|_2}{|\hat{u}_{jj}|}$. Setting $\hat{l}_{ij} = 0$:
\begin{align}
    E_{ij} &= \sum_{k=1}^{j-1}\hat{l}_{ik}\hat{u}_{kj} + \underbrace{\hat{l}_{ij}}_{=0}\hat{u}_{jj} - a_{ij} = -v_{ij}
\end{align}
The drop condition gives $|m_{ij}|\cdot|\hat{u}_{jj}| \le \varepsilon\|A_{\cdot,j}\|_2$, i.e. $\frac{|v_{ij}|}{|\hat{u}_{jj}|}\cdot|\hat{u}_{jj}| = |v_{ij}| \le \varepsilon\|A_{\cdot,j}\|_2$, hence:
\begin{equation}
    |E_{ij}| = |v_{ij}| \le \varepsilon\|A_{\cdot,j}\|_2 \qquad \square
\end{equation}

This result forms the basis of our mixed-precision allocation policy: since we already tolerate a local error of magnitude $\varepsilon\|A_{\cdot,j}\|_2$ by dropping elements, we can similarly tolerate a rounding error of the same order on the elements we retain.
...
\begin{itemize}
    \item A drop tolerance of $\varepsilon = 0.2$ requires 53 GMRES iterations to converge.
    \item A drop tolerance of $\varepsilon = 10^{-4}$ converges extremely rapidly in just 10 iterations.
...
\subsubsection{Heuristic Ternary Search (Dichotomy) Optimization}
To automate the search for an optimal balance, we implemented a heuristic search algorithm (\texttt{opti\_dicho.m}). While commonly referred to as a dichotomy because it halves the search interval at each step, it is technically a form of ternary search: it evaluates the cost function at three points (the boundaries and the midpoint) to locate the minimum without requiring derivatives. 
...
\begin{equation}
    J(\varepsilon) = \text{Time}_{\text{factorization}}(\varepsilon) + \big(\text{Fill-in}(\varepsilon) \times \text{Iterations}(\varepsilon)\big)
\end{equation}
The optimizer successfully located the optimal threshold at $\varepsilon \approx 3.51 \times 10^{-2}$, yielding a robust preconditioner with a fill-in factor of 0.57 and requiring only 4 GMRES iterations under optimal pivoting. Because the search interval is reduced geometrically at each step, the algorithm converges in $O(\log(\frac{1}{\text{tol}}))$ iterations. This logarithmic complexity ensures that only a minimal number of cost function evaluations (each requiring one ILUT factorization and one GMRES solve) are performed, making this offline calibration highly efficient.
...
\subsubsection{Derivation of the Mixed-Precision Criterion}

The global error of the preconditioner decomposes as:
\begin{equation}
    A = \hat{L}\hat{U} + E^{\text{drop}} + E^{\text{round}}
\end{equation}
where $E^{\text{drop}}$ arises from the ILUT dropping rule and $E^{\text{round}}$ from storing retained elements in finite-precision formats. Our goal is to choose the precision $\mu_{ij}$ of each retained element so that $E^{\text{round}}$ does not exceed what is already tolerated by $E^{\text{drop}}$.

\textbf{Step 1 -- Drop error budget.} For any element dropped from column $j$ (either $\hat{u}_{ij}=0$ or $\hat{l}_{ij}=0$), the bound proved in Section~\ref{sec:dropping_bound} gives:
\begin{equation}
    |E_{ij}^{\text{drop}}| \le \varepsilon\|A_{\cdot,j}\|_2
\end{equation}
This quantity $\varepsilon\|A_{\cdot,j}\|_2$ is the \emph{error budget} of column $j$: the maximum local perturbation already accepted by the algorithm.

\textbf{Step 2 -- Rounding error model.} A retained element $x_{ij}$ stored in a format with unit of rounding $\mu_{ij}$ satisfies:
\begin{equation}
    \hat{x}_{ij} = \mathrm{fl}_{\mu_{ij}}(x_{ij}) = x_{ij}(1+\delta_{ij}), \quad |\delta_{ij}| \le \mu_{ij}
\end{equation}
The local rounding error is $E_{ij}^{\text{round}} = x_{ij}\delta_{ij}$, hence $|E_{ij}^{\text{round}}| \le |\hat{x}_{ij}|\,\mu_{ij}$.
However, the \emph{effective} error on the reconstructed product $\hat{L}\hat{U}$ differs between $U$ and $L$ entries, because a rounding error on a multiplier $l_{ij}$ is amplified by the pivot $u_{jj}$:

\begin{itemize}
    \item \textbf{Element $u_{ij} \in U$ ($i \le j$):} $\hat{u}_{ij}$ enters $\hat{L}\hat{U}$ directly, so the effective error is:
    \begin{equation}
        |E_{ij}^{\text{round}}| = |\hat{u}_{ij} - u_{ij}| = |u_{ij}||\delta_{ij}| \le |u_{ij}|\,\mu_{ij}
    \end{equation}

    \item \textbf{Multiplier $l_{ij} \in L$ ($i > j$):} $\hat{l}_{ij}$ is scaled by the pivot $u_{jj}$ in the product, so the effective error on entry $(i,j)$ of $\hat{L}\hat{U}$ is:
    \begin{equation}
        |E_{ij}^{\text{round}}| = |(\hat{l}_{ij} - l_{ij})\,u_{jj}| = |l_{ij}\,\delta_{ij}\,u_{jj}| \le |l_{ij}\,u_{jj}|\,\mu_{ij}
    \end{equation}
\end{itemize}

\textbf{Step 3 -- Preservation constraint.} We impose that the effective rounding error does not exceed the column's error budget:
\begin{equation}
    |E_{ij}^{\text{round}}| \le \varepsilon\|A_{\cdot,j}\|_2
\end{equation}

\textbf{Conclusion -- Unified element-wise precision criterion}
\begin{equation}\label{eq:unified_criterion}
    \boxed{
    \mu_{ij} \le \begin{cases}
        \dfrac{\varepsilon\|A_{\cdot,j}\|_2}{|u_{ij}|}         & \text{if } i \le j \quad (\text{matrix } U) \\[10pt]
        \dfrac{\varepsilon\|A_{\cdot,j}\|_2}{|l_{ij}\,u_{jj}|} & \text{if } i > j  \quad (\text{matrix } L)
    \end{cases}
    }
\end{equation}
The intuition is immediate: the larger the retained element relative to the column budget, the smaller $\mu_{ij}$ must be, requiring higher precision. An element just above the drop threshold yields $\mu_{ij} \approx 1$, allowing FP16 storage. A dominant element far from the threshold demands $\mu_{ij} \ll 1$, imposing FP64. For $L$ entries, the factor $|u_{jj}|$ in the denominator correctly accounts for the amplification of multiplier rounding errors by the pivot before they affect the reconstructed factorization.
...
\begin{enumerate}
    \item \textbf{Pre-calculation:} The column norms $\varepsilon \|A_{\cdot,j}\|_2$ are pre-computed once per column. This requires exactly $2 \times \text{nnz}(A)$ operations for the sum of squares, plus $n$ square roots.
    \item \textbf{Element-wise Evaluation:} For each non-zero element $u_{ij}$ stored in the upper factor $U$, the algorithm performs exactly \textbf{1 division} to evaluate $\tau_{ij}$. For elements $l_{ij}$ in the lower factor $L$, it requires \textbf{1 multiplication} and \textbf{1 division}.
\end{enumerate}
...
Figure~\ref{fig:convergence_fillin_p2} shows the sensitivity analysis on the \textbf{bcsstk08} matrix across different drop tolerances (spanning from loose $\varepsilon = 10^{-1}$ down to strict $\varepsilon = 10^{-10}$). 
...
\begin{figure}[H]
    ...
    \caption{GMRES iteration count (left) and Preconditioner fill-in factor (right) as a function of the drop tolerance (Phase 2, bcsstk08).}
...
At strict tolerances ($\varepsilon \le 10^{-4}$), \textbf{over 90\% of the elements are stored in FP16 (half precision)}, and the remaining elements are stored in the custom FP24 format. Single precision (FP32) is only allocated at even stricter thresholds ($\varepsilon \le 10^{-6}$). This is because a dense preconditioner contains many small-magnitude elements that are close to the drop threshold, allowing a highly relaxed rounding budget. 
As the tolerance becomes loose ($\varepsilon \ge 10^{-1}$), the FP16 fraction shrinks, and the custom FP24 format dominates. In this regime, the few retained elements are large-magnitude entries that are far from the drop threshold, demanding tighter rounding bounds. This dynamically changing distribution proves the elegance of the GEP Mixte criterion: it naturally shifts the representation budget to match the exact mathematical needs of the system.
...
Figure~\ref{fig:memory_footprint} shows the estimated memory footprint in Kilo-Bytes (KB) for GEP Mixte and standard FP64 storage on the \textbf{bcsstk08} matrix. 
...
\begin{figure}[H]
    ...
    \caption{Preconditioner memory footprint (KB) for GEP Mixte versus standard FP64 storage across different drop tolerances (bcsstk08).}
...
Figure~\ref{fig:perf_profile_mem} presents the memory efficiency profiles. Among all threshold-based robust solvers, GEP Mixte (solid blue curve) starts at $\rho = 0.50$ for $\tau = 1$ (in the profile excluding ILU(0)), sharing the top spot with uniform ILUT FP16. However, while ILUT FP16 only converges on half of the matrix suite, GEP Mixte converges on 100\% of the cases and reaches $\rho = 1.0$ at a very tight factor of $\tau \approx 1.3$, demonstrating that it remains either the absolute most memory-efficient or within 30\% of it across all matrices. Standard ILUT variants in FP64 and FP32, while perfectly robust, require substantially more memory, reaching $\rho = 1.0$ only at $\tau \approx 4$---corresponding to a four-fold memory overhead relative to GEP Mixte.

A critical observation concerns uniform ILUT FP16. Although it nominally offers the smallest memory footprint on well-conditioned problems, it exhibits a fundamental numerical instability on a significant fraction of the test suite. Specifically, ILUT FP16 successfully converges on only \textbf{3 out of 6 matrices} (\textit{bcsstk08}, \textit{goddardRocketProblem\_1}, and \textit{tumorAntiAngiogenesis\_8} all diverge), resulting in an asymptotic limit of $\rho_{\infty}(\text{FP16}) \approx 0.50$: its profile curve plateaus and never reaches unity, regardless of how large $\tau$ becomes. This is a direct manifestation of half-precision arithmetic's inability to faithfully represent the preconditioner eigenvalues when significant ill-conditioning is present, a limitation already foreshadowed by the naive multi-precision simulation of Section~3.1. GEP Mixte, by contrast, achieves a \textbf{100\% convergence rate} across all six matrices, matching the absolute robustness of the double-precision reference while compressing the average memory consumption close to the FP32/FP16 regime.
...
Figure~\ref{fig:perf_profile_iter} shows the iteration count profiles. GEP Mixte closely matches the iteration profile of the reference double-precision solver (ILUT FP64), starting at $\rho \approx 0.83$ for $\tau = 1$ and reaching $\rho = 1.0$ at $\tau \approx 1.09$. This indicates that GEP Mixte achieves the absolute optimal iteration count on 5 out of 6 matrices, with a negligible maximum iteration overhead of only 9\% on the remaining system. This proves that the adaptive mixed precision strategy achieves this massive memory compression with zero convergence penalty.

\subsection{The Iteration-Memory Trade-Off}
\label{subsec:pareto}
...
\begin{enumerate}
    \item \textbf{Uniform ILUT FP16 (aggressive but fragile):} Storing the entire preconditioner in FP16 (2 bytes per non-zero) minimizes memory usage. However, as shown by the divergence on several matrices, this uniform compression is too unstable when the matrix is ill-conditioned. The rounding errors in half-precision accumulate and degrade the preconditioner quality, causing GMRES to diverge.

    \item \textbf{Uniform ILUT FP64 (robust but costly):} Storing everything in double-precision (8 bytes per non-zero) offers excellent stability and the lowest iteration counts, converging successfully in 100\% of the cases. However, it imposes a high memory cost, which can slow down large-scale simulations.

    \item \textbf{GEP Mixte (the adaptive compromise):} By dynamically choosing the precision format for each individual element, GEP Mixte gets the best of both worlds. For less sensitive elements, the criterion naturally relaxes to FP16 (which accounts for over 90\% of the entries at strict tolerances), while critical values are kept in FP32 or FP64. This achieves the same 100\% robustness as FP64 while compressing the memory footprint by up to 3.8$\times$.
\end{enumerate}
