...
results_mixte = struct('iters', zeros(n_tests, 1), 'fill_in', zeros(n_tests, 1), ...
    'rel_err', zeros(n_tests, 1), 'precision_dist', zeros(n_tests, 3));
...
fprintf('Seuil    | Mix Iter | Std Iter | ILU Iter | %% 16-bit | %% 32-bit | %% 64-bit\n');
fprintf('---------|----------|----------|----------|----------|----------|----------\n');
...
for i = 1:n_tests
    ...
    % ===== 3. TEST ILU MATLAB =====
    setup.type = 'ilutp'; setup.droptol = s; setup.udiag = 1;
    ...
    fprintf('%.1e | %8d | %8d | %8d | %7.1f%% | %7.1f%% | %7.1f%%\n', ...
        s, res_mix.iterations, res_std.iterations, res_ilu.iterations, pct(1), pct(2), pct(3));
end

fprintf('\n===== GRAPHIQUES GÉNÉRÉS =====\n');

c_mix = '#0072BD'; c_std = '#77AC30'; c_ilu = '#D95319';
...
xlabel('Drop-tolerance'); ylabel('Itérations GMRES');
legend('GEP Mixte', 'GEP Std (64b)', 'ILU MATLAB', 'Location', 'best'); title('Convergence');
...
set(gca, 'XDir', 'reverse'); grid on;
xlabel('Drop-tolerance'); ylabel('Facteur de Fill-in'); title('Évolution du Fill-in');
...
b(2).CData = [0.30 0.75 0.93]; % Cyan (32-bits)
b(3).CData = [0 0.45 0.74];    % Bleu foncé (64-bits)

set(gca, 'XTickLabel', arrayfun(@(x) sprintf('%.1e', x), seuils, 'UniformOutput', false));
xtickangle(45);
ylabel('Proportion des éléments (%)');
xlabel('Drop-tolerance');
legend('FP16 (Half)', 'FP32 (Single)', 'FP64 (Double)', 'Location', 'best');
...
mask_half = (tau >= u_half) & (abs(val_in) <= fp16_max);
mask_single = (tau >= u_single) & ~mask_half;
...
if n_h > 0
    opt.format = 'h';
    val_out(mask_half) = chop(val_in(mask_half), opt);
end
if n_s > 0
    val_out(mask_single) = double(single(val_in(mask_single)));
end
end